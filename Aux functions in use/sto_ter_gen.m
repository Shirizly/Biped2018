...
for i = 2:n % creating the x vector, with stoch. varying horizontal distance
    rr = 1-(1/(n))*(i); % the gradual decay function, so the segments length is decreasing
    
    xp(i) = xp(i-1)+(xbase+rand(1)*rr);   % each segment has a base length and a random addition
end
...
for i = 2:n % creating the y vector, with stoch. varying slope, limited by slo_lim, with a small bias to negate past slope
    ...
    dx = (xp(i)-xp(i-1));  
    zero_bias_rand = (1-2*rand(1)); % random number between -1 and 1
    bias_rand = zero_bias_rand-avg_slo*0.5-slo*0.5; % random number with bias to correct past slope (to keep the terrain height close to zero)
    bias_rand = max(min(bias_rand,1),-1); % limiting the number between 1 and -1, so the slope is still limited by slo_lim
    yp(i) = slo_lim*rr*bias_rand*dx+yp(i-1);
    slo = (yp(i)-yp(i-1))/(xp(i)-xp(i-1)); % slope between 2 last points
    avg_slo = (yp(i)-yp(1))/(xp(i)-xp(1)); % average slope
end
