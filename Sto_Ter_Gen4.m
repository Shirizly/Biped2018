...
%     var = 1/size(y(segstr(i):segstr(i+1)),2)*sum((y(segstr(i):segstr(i+1))-mean(y(segstr(i):segstr(i+1)))*ones(1,length(y(segstr(i):segstr(i+1))))).^2)
end
...
% for i = 1:nseg
%      var = 1/size(yf(segstr(i):segstr(i+1)),2)*sum((yf(segstr(i):segstr(i+1))-mean(yf(segstr(i):segstr(i+1)))*ones(1,length(yf(segstr(i):segstr(i+1))))).^2)
% end    
...
% xlabel('X [m]');
% ylabel('Y [m]');
% title(['Stochastically generated terrain (BL white noise), var = ' num2str(yvar)])
% % axis([0 xend -0.2 0.2])
% axis equal
